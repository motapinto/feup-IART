...
if __name__ == '__main__':
    ...
    while not state == State(2, 0, 4, 3):
        ...
        if state.bucket1 < state.max_bucket1:
            potential_states.insert(0, state.fill(1))
        if state.bucket2 < state.max_bucket2:
            potential_states.insert(0, state.fill(2))
        if state.bucket1 > 0:
            potential_states.insert(0, state.empty(1))
        if state.bucket2 > 0:
            potential_states.insert(0, state.empty(2))
        if state.bucket2 < state.max_bucket2 and state.bucket1 >= (state.max_bucket2 - state.bucket2):
            potential_states.insert(0, state.pour(1))
        if state.bucket2 < state.max_bucket2 and state.bucket1 < (state.max_bucket2 - state.bucket2):
            potential_states.insert(0, state.pour(1))
        if state.bucket1 < state.max_bucket1 and state.bucket2 >= (state.max_bucket1 - state.bucket1):
            potential_states.insert(0, state.pour(2))
        if state.bucket1 < state.max_bucket1 and state.bucket2 < (state.max_bucket1 - state.bucket1):
            potential_states.insert(0, state.pour(2))
